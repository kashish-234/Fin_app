...
export default function Predictions() {
  ...
  const generateAllPredictions = async (profileData = userData) => {
    ...
    await Promise.all([
      generateRetirementPrediction(profileData),
      generateInvestmentPrediction(profileData),
      generateRiskAssessment(profileData),
    ])
  }

  const generateRetirementPrediction = async (profileData = userData) => {
    if (!profileData) return

    try {
      setLoading((prev) => ({ ...prev, retirement: true }))
      const result = await predictionAPI.getRetirementPrediction(profileData)
      setPredictions((prev) => ({ ...prev, retirement: result }))
    } catch (error) {
      console.error("Retirement prediction error:", error)
      setError("Failed to generate retirement prediction")
    } finally {
      setLoading((prev) => ({ ...prev, retirement: false }))
    }
  }

  const generateInvestmentPrediction = async (profileData = userData) => {
    if (!profileData) return

    try {
      setLoading((prev) => ({ ...prev, investment: true }))
      const result = await predictionAPI.getInvestmentRecommendation(profileData)
      setPredictions((prev) => ({ ...prev, investment: result }))
    } catch (error) {
      console.error("Investment prediction error:", error)
      setError("Failed to generate investment prediction")
    } finally {
      setLoading((prev) => ({ ...prev, investment: false }))
    }
  }

  const generateRiskAssessment = async (profileData = userData) => {
    if (!profileData) return

    try {
      setLoading((prev) => ({ ...prev, risk: true }))
      const result = await predictionAPI.getRiskAssessment(profileData)
      setPredictions((prev) => ({ ...prev, risk: result }))
    } catch (error) {
              ...
              {!userData ? (
                <Card>
                  <CardContent className="text-center py-8">
                    ...
                    <Button onClick={() => (window.location.href = "/profile-form")}>Complete Profile</Button>
                  </CardContent>
                </Card>
              ) : (
